...
import { C as CONTENT_LAYER_TYPE, L as LIVE_CONTENT_TYPE, d as defineCollection, a as getCollection } from '../chunks/_astro_content_COSjIQOK.mjs';
import 'js-yaml';
...
import { s as slash } from '../chunks/path_bxFO2Kst.mjs';
...
const config = {
  site: "https://tau-os.freitaseric.com",
  rss: {
    title: "TauOS Blog",
...
async function GET() {
  ...
  return rss({
    ...
    site: config.site,
    items: posts.sort(
      (a, b) => (b.data.updatedAt?.valueOf() ?? b.data.publishedAt.valueOf()) - (a.data.updatedAt?.valueOf() ?? a.data.publishedAt.valueOf())
